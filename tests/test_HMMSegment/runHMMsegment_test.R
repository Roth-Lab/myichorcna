...
# Load in data 
# loaded_copy <- read.csv("test.correctedDepth.txt", sep='\t')
...
# from loaded 
test_chrs <- as.vector(seqnames(gr_loaded_copy))
test_start <- start(gr_loaded_copy)
test_end <- end(gr_loaded_copy)
test_span <- width(gr_loaded_copy)[1]
test_copy <- gr_loaded_copy$copy

# from counts j
chrs <- as.vector(seqnames(tumour_copy[[1]]))
start <- start(tumour_copy[[1]])
end <- end(tumour_copy[[1]])
span <- width(tumour_copy[[1]])[1]
copy <- tumour_copy[[1]]$copy

# check they are equal 
stopifnot(isTRUE(all(test_chrs == chrs)))
stopifnot(isTRUE(all(test_start == start)))
stopifnot(isTRUE(all(test_end == end)))
stopifnot(isTRUE(all.equal(test_copy, copy)))

# create tumour copy test 
# tumour_copy_test <- list()
# id <- 'test_switch'
# tumour_copy_test[[id]] <- gr_loaded_copy
# tumour_copy[id] <- gr_loaded_copy
